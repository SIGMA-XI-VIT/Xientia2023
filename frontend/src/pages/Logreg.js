  ...
  return (
    <div className="card login">
      ...
      <p className="link-login">
        ...
        <span className="switchText" onClick={switchToSignUp}>
          Register Team!
        </span>
      </p>
    </div>
...
function SignupForm({ switchToLogin }) {
  ...
  const handleNumberMemberChange = (e) => {
    ...
    if (currentMembers > maxMembers) setNumberMembers(maxMembers);
    else if (currentMembers < minMembers) setNumberMembers(minMembers);
    else setNumberMembers(currentMembers);
  ...
  return (
    <div className="card signup">
      <div className="form-scroll">
        <h2 className="mb-4">Register Team</h2>
            ...
            <div className="member-scroll">
              {memberList.map((member, index) => (
                <div className="form-group mb-4" key={index}>
                  <label htmlFor={`name${index}`}>{`Member #${
                    index + 1
                  } Name:`}</label>
                  <input
                  ...
                  />
                  <label htmlFor={`regno${index}`}>{`Member #${
                    index + 1
                  } Reg.No:`}</label>
                  <input
                    className="input mb-2"
                  ...
                  />
                  <label htmlFor={`email${index}`}>{`Member #${
                    index + 1
                  } email-id:`}</label>
                  <input
                    className="input mb-2"
        ...
        {showModal && (
          <SuccessModal show={showModal} closeHandler={closeHandler} />
        )}
      </div>
      <div className="overflow-visible mt-2">
        <p className="link-signUp">
          Already registered?
          <span className="switchText" onClick={switchToLogin}>
            Sign in!
          </span>
        </p>
      </div>
  ...
  return (
    <div className="text-center">
      ...
      <p className="link-login mt-10">
        Haven't registered your team yet?
        <span className="switchText" onClick={handleClick}>
          Register Team!
        </span>
      </p>
    </div>
  ...
  return (
    <div className="body">
      {sessionStorage.getItem("isLoggedIn") === "true" ? (
        showLogin ? (
          <LoggedIn switchToSignUp={switchToSignUp} />
        ) : (
